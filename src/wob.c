...
struct wob_buffer *
wob_buffer_create_argb8888(int shmid, struct wob_dimensions dimensions)
{
...
{
	struct wob *state = (struct wob *) data;
...
layer_surface_enter(void *data, struct wl_surface *wl_surface, struct wl_output *entered_output)
{
	...
	// output config
	struct wob_output_config *output_config = wob_config_match_output(app->config, selected_output->name);
...
wl_surface_frame_done(void *data, struct wl_callback *cb, uint32_t time)
{
	...
	struct wob_surface *surface = (struct wob_surface *) data;
